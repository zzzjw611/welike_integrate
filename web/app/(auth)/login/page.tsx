...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(lang === 'zh' ? '登录失败，请重试。' : "Invalid credentials. Please try again.");
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-surface-400 mt-1">
            {lang === 'zh' ? '登录你的 WeLike 工作空间' : 'Sign in to your WeLike workspace'}
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder={lang === 'zh' ? '输入你的密码' : 'Enter your password'}
              className="w-full rounded-lg border border-surface-700 bg-surface-900 px-4 py-2.5 text-sm text-white placeholder:text-surface-500 focus-brand transition-colors"
            />
          </div>
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-2 rounded-lg border border-surface-700 bg-surface-900 px-4 py-2.5 text-sm text-surface-300 hover:bg-surface-800 hover:text-white disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
        >
          {googleLoading ? (
            <Loader2 className="h-4 w-4 animate-spin" />
          ) : (
          ...
          {googleLoading ? (lang === 'zh' ? '正在跳转...' : 'Redirecting...') : 'Google'}
        </button>
